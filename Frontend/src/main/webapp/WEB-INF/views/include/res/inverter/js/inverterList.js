...
$('.tableBody').on('click', 'td[data-detailid]', function(){
	...
	location.href = _cp + "/inverter/detail/"+ trackerGroupId + "/" + detailId;
	
// 체크박스 선택	
}).on('click', 'input[name="inverterId"]', function(){
	...
	var $this = $(this);
	var isChecked = $this.find('input[name="inverterId"]')[0];
...
function del(){
	...
	var check = $('.tableBody').find('input[name="inverterId"]');
	var delCnt = 0;
	...
	var retVal = confirm("선택된 "+ delCnt +"개의 인버터를 삭제합니다.\n삭제 하신 후에는 복구 할 수 없습니다.");
	if (retVal == false) {
		return false;
	}
	...
	$.ajax({
		type:"POST",
		url	: _cp + '/inverter/del',
		data: params,
		success:function() {
			alert('인버터 삭제에 성공 하였습니다.');
			location.replace(_cp + '/inverter/list'); 
		}, error:function(error) {
			alert('인버터 삭제에 실패 하였습니다.');
		}
	});
}
