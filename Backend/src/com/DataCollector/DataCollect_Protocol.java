...
public class DataCollect_Protocol {
	
	private Object ProtocolData;
	
	public DataCollect_Protocol(Integer[] _packet) 
	{	
		if(_packet != null) {
			if(_packet[14] == 'G') {
				switch(_packet.length) {
					case DataCollectManager.SIZE_OF_PROTOCOL_SINGLE_GENERATION:		// 53
						try {
							//ProtocolData = new Protocol_SingleGen();
							 DataCollect_Protocol_SingleGen psg = new DataCollect_Protocol_SingleGen();					 
							...
							DataCollectManager.logger.error("[ DataCollector ] DataCollectProtocol Init. (SINGLE) error occured !  :  " + e.toString());
						}
						break;
					
					case DataCollectManager.SIZE_OF_PROTOCOL_TRIPLE_GENERATION:		// 65
						try {
							DataCollect_Protocol_TripleGen psg = new DataCollect_Protocol_TripleGen();
							...
							// HEADER ( 12 bytes + 4 bytes )
							 ...
							 /* 2 */		psg.GetBody().setPV_V((_packet[21] << 8) + _packet[22]);
							 /* 2 */		psg.GetBody().setPV_A((_packet[23] << 8) + _packet[24]);
							 /* 4 */		psg.GetBody().setPV_Out(Long.parseUnsignedLong(String.format("%02X%02X%02X%02X",(_packet[25]), (_packet[26]), (_packet[27]), _packet[28]),16));
							 ...
							 /* 4 */		psg.GetBody().setCurrentGen(Long.parseUnsignedLong(String.format("%02X%02X%02X%02X",(_packet[41]),(_packet[42]),(_packet[43]),(_packet[44])),16));
							...
							DataCollectManager.logger.error("[ DataCollector ]  DataCollectProtocol Init. (TRIPLE) error occured !  :  " + e.toString());
						}
						break;
			...
			else if(_packet[14] == 'W') {
				if( Utils.split(_packet,0x2C).length == 12 ) 
				try {
					DataCollect_Protocol_Sensor psd = new DataCollect_Protocol_Sensor();
					
					String[] strPacketSplit = Utils.ConvertString(_packet).split(",");
					...
					try { psd.GetBody().setfControllerTemp(Float.parseFloat(strPacketSplit[6]));} catch(NumberFormatException nfe) 	{ psd.GetBody().setfControllerTemp(0.0f);} 
					try { psd.GetBody().setfInnerRltHumid(Float.parseFloat(strPacketSplit[7])); } catch(NumberFormatException nfe) 	{ psd.GetBody().setfInnerRltHumid(0.0f); } 
					try { psd.GetBody().setnHorizontalIDT(Integer.parseInt(strPacketSplit[8])); } catch(NumberFormatException nfe)	{ psd.GetBody().setnHorizontalIDT(0); 	 }
					try { psd.GetBody().setnSlopeIDT(Integer.parseInt(strPacketSplit[9]));		} catch(NumberFormatException nfe)	{ psd.GetBody().setnSlopeIDT(0); 		 }
					try { psd.GetBody().setfWindSpeed(Float.parseFloat(strPacketSplit[10]));	} catch(NumberFormatException nfe)	{ psd.GetBody().setfWindSpeed(0);		 }
					
					psd.GetFooter().setCS(strPacketSplit[11].substring(0,4));
					psd.GetFooter().setETX(strPacketSplit[11].substring(4));
					
					ProtocolData = psd;
				} catch(Exception e) {
					DataCollectManager.logger.error("[ DataCollector ]  DataCollectProtocol Init. (SENSOR) error occured !  :  " + e.toString());
				}
			}
			else if(_packet[14] == 'R') {
				if( Utils.split(_packet, 0x2C).length == 11)
					try {
						DataCollect_Protocol_Shadow psd = new DataCollect_Protocol_Shadow();
						...
						DataCollectManager.logger.error("[ DataCollector ]  DataCollectProtocol Init. (SHADOW) error occured ! : " + e.toString() );
					}
			}
			else if(_packet[14] == 'S') {
				if( Utils.split(_packet, 0x2C).length == 13)
				try {
					DataCollect_Protocol_Shadow_ADC psd = new DataCollect_Protocol_Shadow_ADC();
					...
					psd.GetHeader().setCMD(strPacketSplit[3]);
					try { psd.GetBody().setnADC1(Integer.parseInt(strPacketSplit[4]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC1(0); }
					try { psd.GetBody().setnADC2(Integer.parseInt(strPacketSplit[5]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC2(0); }
					try { psd.GetBody().setnADC3(Integer.parseInt(strPacketSplit[6]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC3(0); }
					try { psd.GetBody().setnADC4(Integer.parseInt(strPacketSplit[7]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC4(0); }
					try { psd.GetBody().setnADC5(Integer.parseInt(strPacketSplit[8]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC5(0); }
					try { psd.GetBody().setnADC6(Integer.parseInt(strPacketSplit[9]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC6(0); }
					try { psd.GetBody().setnADC7(Integer.parseInt(strPacketSplit[10]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC7(0); }
					try { psd.GetBody().setnADC8(Integer.parseInt(strPacketSplit[11]));		} catch(NumberFormatException nfe) { psd.GetBody().setnADC8(0); }
					
					psd.GetFooter().setCS(strPacketSplit[12].substring(0,4));
					psd.GetFooter().setETX(strPacketSplit[12].substring(4));
					
					ProtocolData = psd;
				} catch(Exception e) {
					DataCollectManager.logger.error("[ DataCollector ]  DataCollectProtocol Init. (SHADOWADC) error occured ! : " + e.toString() );
				}
			}
			else if(_packet[14] == 'X') {
				if(Utils.split(_packet, 0x2C).length == 15)
					try {
						DataCollect_Protocol_Status ps = new DataCollect_Protocol_Status();
						
						String[] strPacketSplit = Utils.ConvertString(_packet).split(",");
						
						ps.GetHeader().setSTX(strPacketSplit[0]);
						try { ps.GetHeader().setSI_ID(Integer.parseInt(strPacketSplit[1]));	} catch(NumberFormatException nfe) { ps.GetHeader().setSI_ID(0); }
						try { ps.GetHeader().setTR_ID(Integer.parseInt(strPacketSplit[2]));	} catch(NumberFormatException nfe) { ps.GetHeader().setTR_ID(0); }
						
						ps.GetHeader().setCMD(strPacketSplit[3]);
						try { ps.GetBody().setlCount(Long.parseLong(strPacketSplit[4]));		} catch(NumberFormatException nfe) { ps.GetBody().setlCount(0); }
						try { ps.GetBody().setlX1(Long.parseLong(strPacketSplit[5]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX1(0); }
						try { ps.GetBody().setlX2(Long.parseLong(strPacketSplit[6]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX2(0); }
						try { ps.GetBody().setlX3(Long.parseLong(strPacketSplit[7]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX3(0); }
						try { ps.GetBody().setlX4(Long.parseLong(strPacketSplit[8]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX4(0); }
						try { ps.GetBody().setlX5(Long.parseLong(strPacketSplit[9]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX5(0); }
						try { ps.GetBody().setlX6(Long.parseLong(strPacketSplit[10]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX6(0); }
						try { ps.GetBody().setlX7(Long.parseLong(strPacketSplit[11]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX7(0); }
						try { ps.GetBody().setlX8(Long.parseLong(strPacketSplit[12]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX8(0); }
						try { ps.GetBody().setlX9(Long.parseLong(strPacketSplit[13]));		} catch(NumberFormatException nfe) { ps.GetBody().setlX9(0); }
						
						ps.GetFooter().setCS(strPacketSplit[14].substring(0,4));
						ps.GetFooter().setETX(strPacketSplit[14].substring(4));
						
						ProtocolData = ps;
					} catch(Exception e) {
						DataCollectManager.logger.error("[ DataCollector ]  DataCollectProtocol Init. (X_STATUS) error occured ! : " + e.toString() );
					}
			}
		}
	}
	...
	public synchronized void debug_PrintStatus() {
		...
		case "Protocol_SingleGen":
			...
			for(short s : ((DataCollect_Protocol_SingleGen)ProtocolData).GetHeader().getSTX()) 	{	System.out.format("%c ", s); }
			System.out.println();
			
			System.out.format("SI_ID \t\t: ");
			for(short s : ((DataCollect_Protocol_SingleGen)ProtocolData).GetHeader().getSI_ID()) {	System.out.format("%c ", s); }
			System.out.println();
			
			System.out.format("TR_ID \t\t: ");
			for(short s : ((DataCollect_Protocol_SingleGen)ProtocolData).GetHeader().getTR_ID()) {	System.out.format("%c ", s); }
			System.out.println();
			
			System.out.format("CMD	 \t: %c%n", ((DataCollect_Protocol_SingleGen)ProtocolData).GetHeader().getCMD());
			
			DataCollect_Protocol_SingleGen _psg = (DataCollect_Protocol_SingleGen)ProtocolData;
		...
		case "Protocol_TripleGen":
			...
			for(short s : ((DataCollect_Protocol_TripleGen)ProtocolData).GetHeader().getSTX()) 	{	System.out.format("%c ", s); }
			System.out.println();
			
			System.out.format("CMD	 \t: %c%n", ((DataCollect_Protocol_TripleGen)ProtocolData).GetHeader().getCMD());
			System.out.format("SI_ID \t\t: ");
			for(short s : ((DataCollect_Protocol_TripleGen)ProtocolData).GetHeader().getSI_ID()) {	System.out.format("%c ", s); }
			System.out.println();
			
			System.out.format("TR_ID \t\t: ");
			for(short s : ((DataCollect_Protocol_TripleGen)ProtocolData).GetHeader().getTR_ID()) {	System.out.format("%c ", s); }
			System.out.println();
			
			DataCollect_Protocol_TripleGen _psg_tri = (DataCollect_Protocol_TripleGen)ProtocolData;
